...
class VenusExpress(OrbitalRobot):
    ...
    actions: ActionGroup = ThrustActionGroup(
        ThrustActionCfg(
            asset_name="robot",
            thrusters=(
                # XY translation thrusters (pair-wise, lines-of-action along axes)
                # +X translation (force along +X): direction = (-1, 0, 0)
                ThrusterCfg(
                    offset=(-0.5, 0.0, 0.0),
                ...
                # +Y translation (force along +Y): direction = (0, -1, 0)
                ThrusterCfg(
                    offset=(0.0, -0.5, 0.0),
            ...
            fuel_consumption_rate=(100.0 / (4 * 2000.0)) / 20.0,
        )
    )
...
class CustomCube(OrbitalRobot):
    ## Model
    asset_cfg: RigidObjectCfg = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/robot",
        spawn=UsdFileCfg(
            usd_path="/root/ws/assets/custom/cube_sc.usd",
            activate_contact_sensors=True,
            collision_props=CollisionPropertiesCfg(),
            mesh_collision_props=MeshCollisionPropertiesCfg(
                mesh_approximation="convexDecomposition"
    ...
    ## Actions
    # Thruster order: [ +X, -X, +Y, -Y ]
    # Offsets lie along the same axis as the force direction to avoid torque (r x F = 0)
    actions: ActionGroup = ThrustActionGroup(
        ThrustActionCfg(
            asset_name="robot",
            thrusters=(
                # +X translation (two thrusters, torque-balanced)
                ThrusterCfg(
                    offset=(-0.25, 0.10, 0.0),
                    direction=(-1.0, 0.0, 0.0),
                    power=500.0,
                ),
                ThrusterCfg(
                ...
                # +Y translation (two thrusters, torque-balanced)
                ThrusterCfg(
                    offset=(0.10, -0.25, 0.0),
                    direction=(0.0, -1.0, 0.0),
                    power=500.0,
                ),
                ThrusterCfg(
            ...
            fuel_consumption_rate=(50.0 / (8 * 500.0)) / 20.0,
        )
    )

    ## Frames
    # Use the geometry prim as base so physics APIs are present on the base frame
    # Ensure your child geometry prim is named exactly "Cube" in your USD
    frame_base: Frame = Frame(prim_relpath="Cube")
    # Attach IMU to the cube body so it follows the robot
    frame_imu: Frame | None = Frame(prim_relpath="Cube")
    frame_payload_mount: Frame = Frame(
        prim_relpath="Cube",
        offset=Transform(
            pos=(0.0, 0.0, 0.0),
    ...
    frame_onboard_camera: Frame = Frame(
        prim_relpath="Cube/camera_onboard",
        offset=Transform(
            pos=(1.0, 0.0, 0.0),
            rot=rpy_to_quat(0.0, 0.0, 0.0),
        ),
